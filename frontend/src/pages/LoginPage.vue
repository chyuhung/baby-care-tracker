  ...
  <div class="min-h-dvh bg-bg-main flex flex-col items-center justify-center px-6">
    ...
    <form class="w-full max-w-xs space-y-4" @submit.prevent="submit">
      ...
      <div class="space-y-1">
        ...
        <div class="relative">
          <input v-model="form.password" :type="showPassword ? 'text' : 'password'" :placeholder="isRegister ? '至少6位' : '输入密码'"
            ...
            class="w-full px-4 py-3 pr-12 bg-white border border-border-color rounded-xl text-text-primary placeholder-text-secondary/50 focus:border-primary focus:outline-none transition-colors" />
      ...
      <button type="submit" :disabled="loading"
        class="btn-press w-full py-3.5 bg-primary-deep text-white font-semibold rounded-xl shadow-card hover:shadow-card-hover transition-all disabled:opacity-50 flex items-center justify-center gap-2">
